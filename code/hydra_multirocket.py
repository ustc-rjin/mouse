...
from hydra import HydraMultivariate
import numpy as np
import os
...
class mouse_classifier():
    ...
    def data_split(self,fold):
        #train test split
        '''
        array = np.arange(1,len(train_users)+1)
    ...
    def train(self):
        ...
        for fold in range(5):
            ...
            transform = HydraMultivariate(input_length=self.x_train.shape[-1],num_channels=self.x_train.shape[-2])

            
            self.x_train = transform(torch.Tensor(self.x_train))
            self.x_test = transform(torch.Tensor(self.x_test))
            self.x_train = self.x_train.numpy().astype(dtype=np.float64)
            self.x_test = self.x_test.numpy().astype(dtype=np.float64)
            #'''
            start = time.time()
            self.classifier = Arsenal(rocket_transform="multirocket")
            ...
            print('predicting time:{}, number of samples:{}'.format(time.time()-start,len(self.y_train)+len(self.x_train)))
            #train_eval = [accuracy_score(self.y_train,y_train_pred),recall_score(self.y_train,y_train_pred),precision_score(self.y_train,y_train_pred)]
