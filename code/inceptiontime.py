...
class mouse_classifier():
    ...
    def train(self):
        ...
        for fold in range(5):
            ...
            self.x_train = np.concatenate([self.x_train1,self.x_train2],axis=1)
            self.x_test = np.concatenate([self.x_test1,self.x_test2],axis=1)

            self.classifier = InceptionTimeClassifier(verbose=0,random_state=0,n_epochs=200,file_path='./mouse/trained_models/inceptiontime/',save_best_model=True)

            #'''
            start = time.time()
            self.classifier.fit(self.x_train,self.y_train)
            print('training time:{}'.format(time.time()-start))
            #self.classifier.load_from_path('./mouse/trained_models/inceptiontime/')
